...
export default function App() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const mappedCheckouts: AppConfig[] = (checkoutsData).map(c => ({
        ...
        topBannerImage: c.top_banner_image || '',
  ...
  const handleSaveConfig = async (cfg: AppConfig, asNew = false) => {
    ...
    const payload: any = {
      ...
      top_banner_image: cfg.topBannerImage || '',
  ...
  const handleSetCheckoutActive = async (ids: string[], active: boolean) => {
    if (!supabase || ids.length === 0) return;
    setSavingId('bulk-checkouts');
    try {
      const { error } = await supabase
        .from('checkouts')
        ...
        .in('id', ids);
      if (error) throw error;
      setAllCheckouts(prev => prev.map(checkout => ids.includes(checkout.id) ? { ...checkout, isActive: active } : checkout));
    } catch (err: any) {
      alert('Erro ao atualizar os checkouts:\n' + getErrorMessage(err));
    } finally {
      setSavingId(null);
    }
  };
