...
export const TurmasDashboard: React.FC<TurmasDashboardProps> = ({
    ...
    const turmas = useMemo<TurmaGroup[]>(() => {
        ...
        checkouts.forEach(checkout => {
            ...
            const key = `${title}__${checkout.eventDate || 'sem-data'}`;
    ...
    const availableMonths = useMemo(() => {
        return Array.from(new Set(turmas.map(turma => monthKeyFromDate(turma.date)))).sort((a, b) => b.localeCompare(a));
    }, [turmas]);
    ...
    const filteredTurmas = useMemo(() => {
        ...
        return turmas.filter(turma => {
            ...
            const matchesSearch = !term || [turma.title, turma.location, turma.date, ...turma.checkouts.map(c => c.slug || c.productName)]
                .filter(Boolean)
    ...
    const groupedByMonth = useMemo(() => {
        return filteredTurmas.reduce((acc, turma) => {
            const key = monthKeyFromDate(turma.date);
            if (!acc[key]) acc[key] = [];
            acc[key].push(turma);
            return acc;
        }, {} as Record<string, TurmaGroup[]>);
    }, [filteredTurmas]);
        ...
        <div className="animate-in fade-in duration-500 space-y-8">
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
                <div>
                    ...
                    <p className="text-gray-400 text-sm font-bold mt-1 uppercase tracking-widest">Acompanhe cada evento com checkouts, alunos e vendas</p>
                </div>
                {userRole === 'master' && (
            ...
            <div className="bg-white rounded-[2rem] border border-gray-100 p-4 md:p-5 shadow-sm">
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto_auto] gap-3">
                    <div className="relative">
                        ...
                        <input value={searchTerm} onChange={event => setSearchTerm(event.target.value)} placeholder="Buscar turma, cidade, checkout ou slug..." className="w-full pl-12 pr-4 py-4 rounded-2xl border border-gray-100 bg-gray-50 outline-none focus:ring-2 focus:ring-blue-500 font-bold text-sm" />
                    </div>
            ...
            {Object.keys(groupedByMonth).length === 0 ? (
                <div className="bg-white rounded-[3rem] border border-gray-100 p-12 text-center">
                    ...
                    <p className="text-gray-400 font-bold mt-2">Crie checkouts com nome de turma e data para organizar tudo aqui.</p>
                </div>
            ) : (
                <div className="space-y-5">
                    {Object.entries(groupedByMonth).map(([monthKey, monthTurmas]) => {
                        const isMonthExpanded = expandedMonth === monthKey || expandedMonth === null;
                        return (
                            <div key={monthKey} className="bg-white rounded-[2rem] border border-gray-100 overflow-hidden shadow-sm">
                                <button onClick={() => setExpandedMonth(isMonthExpanded ? 'closed-' + monthKey : monthKey)} className="w-full p-5 md:p-6 flex items-center justify-between bg-gray-50/70 hover:bg-gray-100 transition-all text-left">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center text-white"><Calendar size={20} /></div>
                                        <div><h3 className="text-lg font-black text-gray-900">{monthLabelFromKey(monthKey)}</h3><p className="text-sm text-gray-500 font-bold">{monthTurmas.length} turma(s)</p></div>
                                    </div>
                                    {isMonthExpanded ? <ChevronUp size={22} className="text-gray-400" /> : <ChevronDown size={22} className="text-gray-400" />}
                                </button>

                                {isMonthExpanded && (
                                    <div className="p-4 md:p-5 space-y-4">
                                        {monthTurmas.map(turma => {
                                            ...
                                            return (
                                                <div key={turma.key} className="border border-gray-100 rounded-3xl overflow-hidden">
                                                        ...
                                                        <button onClick={() => setExpandedTurma(isExpanded ? null : turma.key)} className="flex-1 text-left flex items-start gap-4">
                                                            ...
                                                            <div>
                                                                ...
                                                                <div className="mt-2 flex flex-wrap gap-3 text-xs text-gray-500 font-bold">
                                                                    ...
                                                                    {turma.location && <span className="flex items-center gap-1"><MapPin size={13} /> {turma.location}</span>}
                                                                </div>
                                                            </div>
                                                        </button>
