    ...
    for (const lead of turmaLeads) {
      try {
        ...
        if (sendMethod === 'whatsapp') {
          ...
          if (imageUrl) emailBody += `\n📸 Imagem: ${imageUrl}`;
          window.open(`mailto:${lead.email}?subject=${encodeURIComponent(`Seu Ingresso - ${lead.product_name || selectedTurma}`)}&body=${encodeURIComponent(emailBody)}`, '_blank');
        }
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-3xl shadow-lg border border-gray-100 p-8 space-y-6">
          ...
          {sendingStatus !== 'idle' && <div className={`rounded-2xl p-4 border-2 ${sendingStatus === 'sending' ? 'bg-blue-50 border-blue-200' : 'bg-emerald-50 border-emerald-200'}`}><div className="flex items-center gap-3">{sendingStatus === 'sending' ? <Loader2 size={20} className="text-blue-600 animate-spin" /> : <Check size={20} className="text-emerald-600" />}<div className="font-bold text-sm text-gray-900">{statusMessage}</div></div></div>}
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6"><div className="flex items-start gap-3"><AlertCircle size={20} className="text-amber-600 flex-shrink-0 mt-1" /><div className="text-sm text-amber-800"><div className="font-bold mb-2">ℹ️ Como funciona:</div><ul className="space-y-1 text-xs list-disc list-inside"><li>Agora o sistema lista turmas vindas de checkouts e leads.</li><li>Mesmo se o aluno estiver sem turma, ele entra pelo product_id do checkout.</li><li>Cada aluno recebe seu ingresso único.</li></ul></div></div></div>
    </div>
  );
};
